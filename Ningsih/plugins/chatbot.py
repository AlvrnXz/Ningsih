...
from Ningsih import app,BOT_ID
from Ningsih.utils.filter_groups import cbot
from lang import get_command
from Ningsih.utils.lang import language
from Ningsih.mongo import chatb
from Ningsih.plugins.antlangs import get_arg
from Ningsih.utils.custom_filters import admin_filter
...
async def szcbot(_, message: Message):
    ...
    if chat:
        ...
        try:
           ...
           affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Ningsih&ownername=@supunma&user={chat_id}")
           textmsg = (affiliateplus.json()["message"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "Ningsih")
           ...
           trtoen = (message.text if lang=="en" else tr.translate(message.text, dest="en").text).replace(" ", "%20")
           text = trtoen.replace(" ", "%20") if len(message.text) < 2 else trtoen
           safeone = requests.get(f"https://api.safone.tech/chatbot?message={text}&bot_name=Ningsih&bot_master=Supun&user_id={user_id}")
...
AI based chatbot allows Ningsih to talk and provides a more interactive group chat experience.
...
**Language Support**
Ningsih AI chatbot support almost all languages in world .
