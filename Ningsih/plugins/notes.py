...
from Ningsih.mongo.notesdb import Notes, NotesSettings
from Ningsih.utils.cmd_senders import send_cmd
from Ningsih.utils.custom_filters import admin_filter, command, owner_filter
from Ningsih.utils.kbhelpers import rkb as ikb
from Ningsih.utils.msg_types import Types, get_note_type
...
async def get_note_func(c: app, m: Message, note_name, priv_notes_status):
    ...
    if priv_notes_status:
        ...
        return await reply_text(
            ...
            reply_markup=ikb([[("Click Me!",f"https://t.me/szNingsihbot?start=note_{m.chat.id}_{note_hash}","url")]]))
...
async def local_notes(_, m: Message):
    ...
    if curr_pref:
        pm_kb = ikb([[("All Notes",f"https://t.me/ningsihxrobot?start=notes_{grp_id}","url")]])
