...
import pymongo
from Ningsih import app,DB_URI
from Ningsih.utils.filter_groups import nm_g
from lang import get_command
from Ningsih.utils.commands import command
from Ningsih.utils.lang import language
from Ningsih.utils.custom_filters import can_change_filter
...
async def customize_night(client, message: Message, _):
    Ningsih = await message.reply(_["nm2"])
    if message.chat.type == "private":
        return await Ningsih.edit(_["nm5"])
    if message.chat.type == "channel":
        ...
        if len(message.command) < 3:
            return await Ningsih.edit(_["nm11"])
        if "|" in parameter:
            zone, ctime, otime = parameter.split("|")
        else:
            return await Ningsih.edit(_["nm12"])
        zone = zone.strip()
        ctime = ctime.strip()
        otime = otime.strip()
        if len(ctime) != 11:
            return await Ningsih.edit(_["nm13"])
        if len(otime) != 11:
            return await Ningsih.edit(_["nm13"])
        if not zone and ctime and otime:
            return await Ningsih.edit(_["nm14"])
        ttime = dateparser.parse("now", settings={"TIMEZONE": f"{zone}", "DATE_ORDER": "YMD"})
        if ttime is None or otime is None or ctime is None:
            return await Ningsih.edit("Please enter valid `date`, `time` & `zone`")
        ...
        if cctime == ootime:
            return await Ningsih.edit(_["nm15"])
        if not ootime > cctime and not cctime < ootime:
            return await Ningsih.edit(_["nm16"])
        if cctime > ootime:
            return await Ningsih.edit("Chat closing time can't be greater than opening time")
        chats = nightmod.find({})
        for c in chats:
            if message.chat.id == c["id"] and c["valid"] is True:
                ...
                await Ningsih.edit("**Nightmode already set**\n\n__I am updating the zone, closing time and opening time with the new zone, closing time and opening time__")
                await asyncio.sleep(2)
                return await Ningsih.edit(f"**Nightmode Updated Successfully in {message.chat.title} chat**")
        nightmod.insert_one({"id": message.chat.id,"valid": True,"zone": zone,"ctime": cctime,"otime": ootime})
        await Ningsih.edit(f"**Nightmode set successfully in {message.chat.title} chat !**")
